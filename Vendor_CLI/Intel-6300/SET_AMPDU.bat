...
REG ADD HKEY_LOCAL_MACHINE\%myOutput% /v tlcAutoAggregation /t REG_DWORD /d %flag% /f
...
echo: syntax Error
echo:
echo:Example = Try - "SET_AMPDU /interface <interface name> /action <Enable/Disable> [/NoDisable]"
echo:
...
:Success
echo:
echo:A-MPDU Aggregation are now %4%d
echo: 
