...
REG ADD HKEY_LOCAL_MACHINE\%myOutput% /v ShortGICapabilities /t REG_DWORD /d %flag% /f
...
echo: syntax Error
echo:
echo:Example = Try - "SET_SGI /interface <interface name> /action <Enable/Disable> [/NoDisable]"
echo:
...
:Success
echo:
echo:Short GI capabilities are now %4%d
echo: 
