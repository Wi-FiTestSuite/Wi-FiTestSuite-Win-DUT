...
//
// History
// 	Modified for  P2P - Windows 7  -Chandra Sekhar Duba
        ...
        if ((argv[i][0] == '-') || (argv[i][0] == '/'))
        {
            switch (tolower(argv[i][1]))
            {
				...
				case 'p':       // traffic priority
					    ...
					    if(strcasecmp("vo", priostr) == 0)
						{
							tosprio = TG_WMM_AC_VO;
						}
						else if( strcasecmp("vi", priostr) == 0)
						{
							tosprio = TG_WMM_AC_VI;
						}
						else if(strcasecmp("be", priostr) == 0)
						{
							tosprio = TG_WMM_AC_BE;
						}
						else if(strcasecmp("bk", priostr) == 0)
						{
							tosprio = TG_WMM_AC_BK;
						}
						else
						{
							usage(argv[0]);
...
//    for(i=0; i < DEFAULT_SEND_COUNT ;i++)
	...
	for(i=0; i < gMaxCount ;i++)
    {
		...
		//printf(".......sent waiting for wait time");
...
CLEANUP:
    ...
    //
    if (WSAGetLastError() != WSA_IO_PENDING)
        //printf(stderr, "WSAGetLastError: %d\n", WSAGetLastError());
		;
	else
	{
		//printf("\n WSARecvfrom - status -WSA_IO_PENDING");
		return 0;
	}
    if (dest)
         freeaddrinfo(dest);
...
int wfaTGSetPrio(int sockfd, int tgClass)
{
    ...
    switch(tgClass)
    {
       ...
       /*Change this value to the ported device*/
	   if(vend == WMM_ATHEROS ||vend == WMM_MARVELL )
		   tosval=0xe0;
	   else
       tosval = TOS_VO;
       //tosval = 0x30;
